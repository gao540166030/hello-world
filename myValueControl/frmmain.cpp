...
void frmMain::UpdateData2()
{
    ...
    ui->labValue->setText(tr("当前值:温度%1℃ 湿度%2RH").arg(value).arg(value2));
}
...
void frmMain::UpdateData6()
{
    ...
    QFile file("/home/pi/mytest/goodata/test.txt");
     ...
     if (!line.isNull()) {
         QStringList strs = line.split(",");
         QString valuestr = strs.at(1).mid(0);
         value3 = valuestr.toInt();
     }
     file.close();
    gauge61->setValue(value3);
    gauge62->setValue(value3);
    gauge63->setValue(value3);
    ui->labValue->setText(tr("当前值:%1").arg(value3));
}
